...
// STM32PLUS: we softcode the buffer count and allocate memory dynamically
...
/**
  ...
  * @retval Structure of type FrameTypeDef
  */
FrameTypeDef ETH_Get_Received_Frame_interrupt(uint8_t receiveBufferCount)
{ 
  FrameTypeDef frame={0,0,0};
  ...
  while (((DMARxDescToGet->Status & ETH_DMARxDesc_OWN) == (uint32_t)RESET)&&
        (descriptor_scan_counter<receiveBufferCount))
  {
